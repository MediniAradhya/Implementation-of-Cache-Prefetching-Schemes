...
#define BUFFER_BLK_VALID    0x00000003
#define BUFFER_BLK_DIRTY    0x00000004 
...
//{
 ...
 //struct buffer_blk_t *way_next;	/* next block in the ordered way chain, used
				   //to order blocks for replacement */
  //struct buffer_blk_t *way_prev;	/* previous block in the order way chain */
  //struct cache_blk_t *hash_next;/* next block in the hash bucket chain, only
				   //used in highly-associative caches */
  /* since hash table lists are typically small, there is no previous
     pointer, deletion requires a trip through the hash table bucket list */
  ...
  //unsigned int status;		/* block status, see CACHE_BLK_* defs above */
  //tick_t ready;		/* time when block will be accessible, field
				   //i/s set when a miss fetch is initiated */
  //byte_t *user_data;		/* pointer to user defined data, e.g.,
				   //pre-decode data or physical page address */
  /* DATA should be pointer-aligned due to preceeding field */
  /* NOTE: this is a variable-size tail array, this must be the LAST field
...

{
  ...
  struct buffer_blk_t *way_head;
  struct buffer_blk_t *way_tail;
  struct buffer_blk_t *blks;
  struct buffer_blk_t *last_blk;
...
struct cache_set_t
{
 ...
 //counter_t fetch_count;
//struct cache_blk_t *stream_buff;
...
//struct buffer_t *			/* pointer to buffer created */
//buffer_create(int bsize,	/* block (line) size of buffer */
	     //int balloc,		/* allocate data space for blocks? */
	     //int usize,			/* size of user data to alloc w/blks */
	     /* block access function, see description w/in struct buffer def */
	     //unsigned int (*blk_access_fn)(enum mem_cmd cmd,
					   //md_addr_t baddr, int bsize,
					   //struct buffer_blk_t *blk,
					   //tick_t now),
	     //unsigned int hit_latency);/* latency in cycles for a hit */
