...
#define BUFFER_TAG(buff,addr) ((addr) >> (cp)->tag_shift)
#define BUFFER_BLK(buff, addr)	((addr) & (cp)->blk_mask)
#define BUFFER_TAGSET(buff, addr) ((addr) & (cp)->tagset_mask)
...
#define CACHE_MK_BADDR(cp, tag, set)					\
 ...
 #define BUFFER_BADDR(buff, addr)	((addr) & ~(cp)->blk_mask)
 #define BUFFER_MK_BADDR(cp, tag)					\
  ((tag) << (cp)->tag_shift)
...
#define BUFFER_BINDEX(cp, blks, i)					\
  ((struct buffer_blk_t *)(((char *)(blks)) +				\
			  (i)*(sizeof(struct buffer_blk_t) +		\
			       ((cp)->balloc				\
				? (cp)->bsize*sizeof(byte_t) : 0))))				
...
struct cache_blk_t *blk_b;
  ...
  cp = (struct cache_t *)
    calloc(1, sizeof(struct cache_t) + (nsets)*sizeof(struct cache_set_t));
  if (!cp)
    fatal("out of virtual memory");
 //cp->buff = (struct buffer_t *)
    //calloc(1, sizeof(struct buffer_t));
	//if (!cp->buff)
    //fatal("out of virtual memory");
  ...
  cp->hit_latency = hit_latency;
	//cp->buff->hit_latency=hit_latency;
  /* miss/replacement functions */
  cp->blk_access_fn = blk_access_fn;
  //cp->buff->blk_access_fn=blk_access_fn;

  /* compute derived parameters */
  cp->hsize = CACHE_HIGHLY_ASSOC(cp) ? (assoc >> 2) : 0;
  cp->blk_mask = bsize-1;
  //cp->buff->blk_mask=bsize-1;
  cp->set_shift = log_base2(bsize);
  //cp->buff->set_shift=log_base2(bsize);
  cp->set_mask = nsets-1;
  cp->tag_shift = cp->set_shift + log_base2(nsets);
  //cp->buff->tag_shift=cp->buff->set_shift + log_base2(nsets);
  cp->tag_mask = (1 << (32 - cp->tag_shift))-1;
  //cp->buff->tag_mask = (1 << (32 - cp->buff->tag_shift))-1;
  cp->tagset_mask = ~cp->blk_mask;
  //cp->buff->tagset_mask = ~cp->buff->blk_mask;
  cp->bus_free = 0;
  //cp->buff->bus_free=0;
  ...
  //cp->buff = buffer_create(nsets,bsize,balloc,usize,assoc,blk_access_fn,hit_latency);
  ...
  cp->misses = 0;
  //cp->buff->misses=0;
 // cp->buff->replacements=0;
  cp->replacements = 0;
  cp->writebacks = 0;
  //cp->buff->writebacks=0;
  cp->invalidations = 0;
  //cp->buff->invalidations=0;
  ...
  cp->data = (byte_t *)calloc(3*nsets * assoc,
			      sizeof(struct cache_blk_t) +
			      (cp->balloc ? (bsize*sizeof(byte_t)) : 0));
  if (!cp->data)
    fatal("out of virtual memory");

/* cp->buff.data = (byte_t *)calloc(nsets * assoc,
			      sizeof(struct buffer_blk_t) +
			      (cp->balloc ? (bsize*sizeof(byte_t)) : 0));
	if (!cp->buff.data)
	 fatal("out of virtual memory"); */

  /* slice up the data blocks */
  for (bindex=0,i=0; i<nsets+1; i++)
    {
	...
	{
	cp->buff.blks=CACHE_BINDEX(cp, cp->data, bindex);
	cp->buff.way_head = NULL;
     cp->buff.way_tail = NULL;

  /* slice up the data blocks */
  for (j=0; j<nsets*assoc; j++)
    {
      
	  /* locate next cache block */
	  blk_b = CACHE_BINDEX(cp, cp->data, bindex);
		bindex++;
	  /* invalidate new buffer block */
	  blk_b->status = 0;
	  blk_b->tag = 0;
	  blk_b->ready = 0;
	  blk_b->user_data = (usize != 0
			    ? (byte_t *)calloc(usize, sizeof(byte_t)) : NULL);


	  /* insert into head of way list, order is arbitrary at this point */
	  blk_b->way_next = cp->buff.way_head;
	  blk_b->way_prev = NULL;
	  if (cp->buff.way_head)
	    cp->buff.way_head->way_prev = blk_b;
	  cp->buff.way_head = blk_b;
	  if (!cp->buff.way_tail)
	    cp->buff.way_tail = blk_b;
    }
	}
	}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  md_addr_t tagnext = CACHE_TAG(cp,addr+1);
  md_addr_t set = CACHE_SET(cp, addr);
  md_addr_t bofs = CACHE_BLK(cp, addr);
  struct cache_blk_t *blk, *repl, *blk_buff;

  
  int lat = 0;
blk_buff=cp->buff.way_head;
  ...
  //check buffer
		  ...
		  if(blk_buff->tag == tag &&(blk_buff->status & CACHE_BLK_VALID))
			  goto buffer_hit;
  ...
  repl->status = CACHE_BLK_VALID;	/* dirty bit set on update */
	blk_buff->tag = tagnext;
	blk_buff->status = CACHE_BLK_VALID;
  /* read data block */
  lat += cp->blk_access_fn(Read, CACHE_BADDR(cp, addr), cp->bsize,
			   repl, now+lat);
	//block look ahead on a miss, wait for bus to be available
	
	lat+=BOUND_POS(cp->bus_free - (now+lat));
	//lat+=cp->buff->blk_access_fn(Read, CACHE_BADDR(cp,addr+1),cp->bsize,
				//blk_b,now+lat);
	//track bus resource usage */
	cp->bus_free = MAX(cp->bus_free, (now + lat)) +1;
...
blk_buff->ready=now+lat;
 ...
 /* select the appropriate block to replace, and re-link this entry to
  ...
  repl->status = CACHE_BLK_VALID;	/* dirty bit set on update */
	...
	cp->last_tagset=CACHE_TAGSET(cp,addr);
	cp->last_blk=blk_buff;
	 
	 //blk_b->way_next->tag=tagnext;
	 //Stall until bus to next level of memory is free
	 //lat += BOUND_POS(cp->bus_free - (now + lat));
	 
	 //cp->bus_free = MAX(cp->bus_free, (now + lat)) + 1;
	
	//blk_b->way_next->ready=now+lat;
	return lat;
